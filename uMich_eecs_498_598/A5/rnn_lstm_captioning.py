...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # RNN forward formula: h_t = tanh(x_t @ Wx + h_{t-1} @ Wh + b)
    # x: (N, D), Wx: (D, H) -> x @ Wx: (N, H)
    # prev_h: (N, H), Wh: (H, H) -> prev_h @ Wh: (N, H)
    # b: (H,) -> broadcast to (N, H)
    hidden = x @ Wx + prev_h @ Wh + b
...
def rnn_step_backward(dnext_h, cache):
    ...
    # tanh的导数: d_tanh(z) = 1 - tanh(z)^2
    # 这里next_h = tanh(z)，所以d_tanh = 1 - next_h^2
    dhidden = (1 - torch.tanh(hidden)**2) * dnext_h# shape: (N, H)
    
    # 对于 hidden = x @ Wx + prev_h @ Wh + b
    dWx = x.T @ dhidden  # shape: (D, H)

    dWh = prev_h.T @ dhidden  # shape: (H, H)
    
    db = torch.sum(dhidden, dim=0)  # shape: (H,)
    
    dx = dhidden @ Wx.T  # shape: (N, D)
    
    dprev_h = dhidden @ Wh.T  # shape: (N, H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_i = x[:, i, :]
        next_h, cache_t = rnn_step_forward(x_i, prev_h, Wx, Wh, b)
        h[:, i, :] = next_h
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # input: [batch_size, num_classes], target: [batch_size]
    # 在时间步上求和, 在批次上平均
    loss = torch.nn.functional.cross_entropy(x.reshape(-1, x.size(-1)),
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # 3. 根据cell_type选择backbone
        # wordvec_dim : input, hidden : output
        if cell_type == 'rnn' :
    ...
    def forward(self, images, captions):
        ...
        #     of shape (N, H, 4, 4).
        # (2) Use a word embedding layer to transfor m the words in captions_in
        ...
        # 词嵌入
        word_vec = self.word_embedding(captions_in)

        # 序列处理
        # RNN/LSTM  将图像特征图 [H, W, C] 展平为 [H*W*C]
        if self.cell_type in ['rnn' ,'lstm']:
            h0 = self.feature_proj(x.reshape(x.shape[0], -1))   # (N, hidden_dim)
            hT = self.backbone(word_vec, h0)
        else :
            # Attention LSTM: 保持空间结构进行投影
            A = self.feature_proj(x.permute(0,2,3,1)).permute(0,3,1,2) #(N,C,H',W') -> (N,H',W', hidden) ->(N,hidden, H' ,W')
            hT = self.backbone(word_vec, A)
    ...
    def sample(self, images, max_length=15):
        ...
        # 2. 词语生成循环
        # 起始输入,创建一个与 images 张量在相同设备上、相同数据类型的张量
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        a_i, a_f, a_o, a_g = torch.split(A, H, dim=1)
        i = torch.nn.functional.sigmoid(a_i)
        f = torch.nn.functional.sigmoid(a_f)
        o = torch.nn.functional.sigmoid(a_o)
        g = torch.nn.functional.tanh(a_g)
        next_c = f * prev_c + i * g #逐个元素相乘 or torch.mul
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        N, T, D = x.shape
        _ , H = h0.shape
        hn = torch.zeros(N, T, H, device=h0.device)
        prev_h = h0
        prev_c = c0
        for i in range(T):
            x_i = x[:, i, :]
            prev_h, prev_c = self.step_forward(x_i, prev_h, prev_c)
            hn[:, i, :] = prev_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    A_flat = A.reshape(N, H, -1).permute(0, 2, 1) #(N, 16, H)
    
    E = (A_flat @ prev_h.unsqueeze(-1) ) .squeeze(-1) / math.sqrt(H)  #(N, 16)
    attn_weights = torch.nn.functional.softmax(E, dim=1) #(N, 16)
    attn = torch.sum(attn_weights.unsqueeze(1) @ A_flat, dim=1)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for i in range(T):
            ...
            prev_h, prev_c = self.step_forward(x_i, prev_h, prev_c, attn)
            hn[:, i, :] = prev_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
