...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = torch.zeros(K, M, device=query.device)
    for i in range(K):
        scores = torch.zeros(K, device=query.device)
        for j in  range(K):
            E = query[i] @ key[j] / (M ** 0.5) 
            scores[j] = E

        weights = F.softmax(scores, dim=0)  #scalar
        for k in range(K):
            out[i] += weights[k] * value[k]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    scores = torch.zeros(N, K, K, dtype=query.dtype, device=query.device)  #i.e. E
    for i in range(K):
        for j in range(K):
            query_i = query[:, i , :]   #(N, M)
            key_j = key[:, j , :]       #(N, M)
            dot_product_batch = query_i * key_j / (M ** 0.5)        #(N, M)
            score = torch.sum(dot_product_batch, dim=1)     #(N,)
            scores[:, i , j] = score
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scores = query @ key.permute(0,2,1) / (M ** 0.5)
    if mask is not None:
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        for head in self.heads:
            out.append(head(query, key, value, mask))
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        sigma = x.var(dim=-1, keepdim=True, unbiased=False)     #unbiased=False：使用 n 作为分母, 有偏估计

        x_norm = (x - mean ) / ((sigma + self.epsilon) ** 0.5 )
...
class Decoder(nn.Module):
    ...
    def forward(self, target_seq: Tensor, enc_out: Tensor, mask: Tensor):
        # target_seq: (N, O-1, emb_dim)

        out = target_seq.clone()    # (N, O-1, emb_dim)
        
        for _layer in self.layers:
            out = _layer(out, enc_out, mask)

        # 最后投影到词汇表大小   
        out = self.proj_to_vocab(out)   # (N, O-1, vocab_len)
        return out
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.zeros(1, K, M)
    # t = torch.linspace(0, (K-1) / K, steps=K).reshape(1, K)
    # for i in range(M):
    #     y[:, :, i] = t
    for i in range(K):
        y[0, i, :] = i / K if K > 1 else 0
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    pe = torch.zeros(K, M)
    pos = torch.arange(0, K, dtype=torch.float).reshape(K, 1)

    # 计算指数部分：i/d_model
    i = torch.arange(0, M, 2)
    # //整除才能通过检验，为什么需要整除？
    div_term = torch.exp(-math.log(10000) * (i // M ))
    ...
    # k = torch.arange(K).view(-1, 1)     #(k, 1)
    # m = torch.arange(M).view(1, -1)     #(1, M)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        args:
            ...
            ans_b: Tensor of shape (N, O) that consists of target sequence
                of arithmetic expression
            ans_pos: Tensor of shape (N, O, M) that consists of positonal
                encodings of the target sequence

        returns:
            dec_out: Tensor of shape (N*(O-1), vocab_len) where O is the size of
                the target sequence.
        """
        q_emb = self.emb_layer(ques_b)
        a_emb = self.emb_layer(ans_b)
        q_emb_inp = q_emb + ques_pos
        a_emb_inp = a_emb[:, :-1] + ans_pos[:, :-1] #用前 t-1 个符号预测第 t 个符号
        dec_out = None
        ##########################################################################
        ...
        # a_emb_inp ：(N, O-1, emb_dim)
        N, O_minus1, M = a_emb_inp.shape
        # decoder 输出形状：(N, O-1, vocab_len)
        dec_out = self.decoder(a_emb_inp, enc_out, mask).reshape(N*O_minus1, -1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
