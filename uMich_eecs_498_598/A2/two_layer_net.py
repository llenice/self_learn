...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    H = W1.shape[1]
    device = y1.device
    zeros = torch.zeros(N, H, device=device)
...
def nn_forward_backward(
    ...
    # 只需真实标签对应的probs
    corret_log_probs = -torch.log(probs[range(N), y])

    loss = torch.sum(corret_log_probs) / N
    ...
    gradL_W2 = h1.t().mm(gradL_s)  + 2 * reg *W2

    gradL_b2 = gradL_s.sum(dim=0)

    gradL_h1 =  gradL_s.mm( W2.t())
    gradL_h1[h1 <= 0] = 0

    gradL_W1 = X.t().mm(gradL_h1) + 2 *reg * W1
    gradL_b1 = gradL_h1.sum(dim=0)

    grads["W2"], grads["b2"], grads["W1"], grads["b1"] = gradL_W2, gradL_b2, gradL_W1, gradL_b1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def find_best_net(
    ...
    input_size = data_dict['X_train'].shape[1]  # D
    output_size = max(data_dict['y_train']) + 1  #C
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decays in learning_rate_decays:
                    ...
                    train_stats = net.train(data_dict["X_train"], data_dict["y_train"],
                                              data_dict['X_val'], data_dict['y_val'],
                                              lrs, lr_decays, reg ,1000, 1000)    #num_iter batch_size,  重要！
                    
                    val_acc= train_stats['val_acc_history'][-1]
