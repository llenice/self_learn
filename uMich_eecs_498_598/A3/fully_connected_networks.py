...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.normal(0.0, std=weight_scale, 
                                         size=(input_dim, hidden_dim), 
                                         dtype=dtype, device=device)
        
        self.params['b1'] = torch.zeros(hidden_dim, dtype=dtype, device=device)

        self.params['W2'] = torch.normal(0.0, std=weight_scale, 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        L = self.num_layers
        for i in range(2, L):
            self.params['W{0}'.format(i)] = torch.normal(0.0, weight_scale, 
    ...
    def loss(self, X, y=None):
        ...
        L = self.num_layers
        cache, cache_dropout = [], []
        out, cache0 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        cache.append(cache0)
        if self.use_dropout :
            out, cache1 = Dropout.forward(out, self.dropout_param)
            cache_dropout.append(cache1)
        for i in range(2, L):
            out, cache0 = Linear_ReLU.forward(out, self.params[f'W{i}'], self.params[f'b{i}'])
            cache.append(cache0)
            if self.use_dropout :
                out, cache1 = Dropout.forward(out, self.dropout_param)
                cache_dropout.append(cache1)
            
        scores, cache2 = Linear.forward(out, self.params[f'W{L}'], self.params[f'b{L}'])
        cache.append(cache2)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(L-1, 0, -1):
            if self.use_dropout:
              dh = Dropout.backward(dh, cache_dropout[i-1])  #cache_dropout: L-1 
...
class Dropout(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            # Replace "pass" statement with your code
            dx = mask / (1 - dropout_param['p']) * dout
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
        elif mode == 'test':
