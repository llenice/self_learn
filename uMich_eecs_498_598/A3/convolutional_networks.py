...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    for i in range(H_p):
      for j in range(W_p):
        x_slice = x_padded[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW].reshape(N,-1)
  ...
  def backward(dout, cache):
    ...
    stride = conv_param['stride']
    pad = conv_param['pad']
    _, _, H_p, W_p = dout.shape #(N,F,H',W')
    p2d = (pad, pad, pad, pad)
    x_padded = torch.nn.functional.pad(x,p2d)
    ...
    for i in range(H_p):
      for j in range(W_p):
        ddout = dout[:,:,i,j].reshape(N,-1) #(N,F)
        x_slice = x_padded[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW].reshape(N,-1) #(N,C,HH,WW)
        dw+= ddout.t().mm(x_slice).reshape(F,C,HH,WW)
        dx[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW] += ddout.mm(w.reshape(F,-1)).reshape(N,C,HH,WW)
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    H_p = 1 + (H - HH) // stride
    W_p = 1 + (W - WW) // stride
    out = torch.zeros(N, C, H_p, W_p, dtype=x.dtype, device=x.device)
    for i in range(H_p):
      for j in range(W_p):
        x_slice = x[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW].reshape(N,C,-1)
        out[:,:,i,j],_ = x_slice.max(dim=2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(H_p):
      for j in range(W_p):
        x_slice = x[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW].reshape(N*C,-1)
        idx = x_slice.argmax(1).reshape(N*C,1) # shape (N*C,)
        dout_slice = dout[:,:,i,j].reshape(N*C,1) #shape (N*C)
        dx_slice = torch.zeros_like(x_slice).scatter(1,idx,dout_slice)
        dx[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW] = dx_slice.reshape(N,C,HH,WW)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    W1 = torch.randn([num_filters, input_dims[0], filter_size, filter_size], dtype=dtype, device=device)*weight_scale
    ...
    W2 = torch.randn([num_filters*H_p*W_p, hidden_dim], dtype=dtype, device=device)*weight_scale
  ...
  def loss(self, X, y=None):
    ...
    grads['b2'] = db
    dx, dw, db = Conv_ReLU_Pool.backward(dout.reshape(*out1.shape), cache1)
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for layer,F in enumerate(num_filters):
      if weight_scale == 'kaiming':
        self.params[f'W{layer}'] = kaiming_initializer(inp_filters, F, K=filter_size, dtype=dtype, device=device)
      else:
        self.params[f'W{layer}'] = torch.randn([F, inp_filters, filter_size, filter_size], dtype=dtype, device=device)* weight_scale
      if batchnorm:
    ...
    #last Linear layer
    H_p, W_p = H // 2**len(max_pools), W // 2**len(max_pools)  
    layer = self.num_layers-1
    if weight_scale == 'kaiming':
      self.params[f'W{layer}'] = kaiming_initializer(inp_filters*H_p*W_p, num_classes, dtype=dtype, device=device)
    else:
      self.params[f'W{layer}'] = torch.randn([inp_filters*H_p*W_p, num_classes], dtype=dtype, device=device)* weight_scale
    self.params[f'b{layer}'] = torch.zeros(num_classes, dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i == self.num_layers-1:
        ...
        use_maxpool = i in self.max_pools
        if self.batchnorm:
        ...
        if use_maxpool:
          if self.batchnorm:
            ...
            scores, cache = Conv_BatchNorm_ReLU_Pool.forward(scores, W, b, gamma, beta, conv_param, bn_param, pool_param)
          else:
            scores, cache = Conv_ReLU_Pool.forward(scores, W, b, conv_param, pool_param)
        else:
          if self.batchnorm:
            bn_param = self.bn_params[i]
            scores, cache = Conv_BatchNorm_ReLU.forward(scores, W, b, gamma, beta, conv_param, bn_param)
          else:
    ...
    for i in range(self.num_layers-1, -1, -1):
      ...
      if i == self.num_layers-1:
        dout, dw, db = Linear.backward(dout, cache_dict[i])
      else:
        use_maxpool = i in self.max_pools
        if use_maxpool:
          if self.batchnorm:
            dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, cache_dict[i])
          else:
            dout, dw, db = Conv_ReLU_Pool.backward(dout, cache_dict[i])
        else:
          if self.batchnorm:
            dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache_dict[i])
          else:
            dout, dw, db = Conv_ReLU.backward(dout, cache_dict[i])
        if self.batchnorm:
...
def find_overfit_parameters():
  ...
  # weight_scale = 1.85e-1   # Experiment with this!
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims=input_dims, num_classes=10,
                      ...
                      max_pools=[1, 3, 5],
                      reg=1e-5, weight_scale='kaiming', dtype=dtype, device=device)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    # Replace "pass" statement with your code
    fan_in = Din
    scale = torch.sqrt(torch.as_tensor(gain / fan_in, dtype=dtype, device=device))
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      mu = x.mean(dim=0)  #step1
      x_mu = x - mu #step2
      sq = x_mu **2 #step3
      var = sq.mean(dim=0) #step4
      sigma = (var+eps).sqrt() #step5
      invsigma = 1/sigma #step6
      xhat = x_mu * invsigma #step7
      out = xhat * gamma + beta #step8
      ...
      #######################################################################
      # Replace "pass" statement with your code
      out = (x-running_mean)/torch.sqrt(running_var+eps)*gamma+beta
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    # step 7
    # xhat = x_mu * invsigma #step7
    dx_mu1 = dxhat * invsigma
    dinvsigma = (dxhat * x_mu).sum(0)

    # step 6
    # invsigma = 1/sigma #step6
    dsigma = - invsigma**2 * dinvsigma

    #step 5
    # sigma = (var+eps).sqrt() #step5
    dvar = 1/2*invsigma * dsigma

    #step 4
    # var = sq.mean(dim=0) #step4
    dsq = (1/N*dvar).expand(N,D)
  ...
  def backward_alt(dout, cache):
    ...
    dbeta = dout.sum(0)
    dxhat = dout * gamma
    dx = 1/N * invsigma * (N * dxhat - dxhat.sum(0) - xhat * (dxhat*xhat).sum(0))
...
class SpatialBatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dout_new = dout.permute(0,2,3,1).reshape(N*H*W,C)
    dx, dgamma, dbeta = BatchNorm.backward(dout_new, cache)
