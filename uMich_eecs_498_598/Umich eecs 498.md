...
| :--------------------- | :--------------------- | :----------------------------------------------------------- |
| **全连接层（Dense）**  | ✅              | 参数量大，易过拟合。通常在每个全连接层后添加，如 `FC → Dropout → ReLU`。 |
| **卷积层（CNN）**      | ⚠️            | 参数共享降低过拟合风险。若需使用，建议在最后 1-2 个卷积层后添加。 |
| **循环层（RNN/LSTM）** | ✅                 | 隐状态易过拟合，通常在循环层之间或输出前添加（如 `LSTM → Dropout`）。 |
| **输入层**             | ✅ 低概率           | 丢弃率 *p* 通常较低（如 0.1-0.2），防止输入信息丢失过多。    |
| **输出层**             | ❌                | 会导致预测不稳定，破坏输出概率分布。                         |
