...
def compute_saliency_maps(X, y, model):
    ...
    correct_scores = torch.gather(scores, dim=1, index=y.view(-1, 1)).squeeze() #(N, 1) → (N,)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        correct_scores = outputs[0, target_y]
        correct_scores.backward()

        norm_grad = torch.norm(X_adv.grad)
        X_adv.grad =  X_adv.grad / norm_grad

        X_adv.data += learning_rate * X_adv.grad

        if verbose and (i + 1) % 10 == 0:
            current_pred = idx.item()
            target_score_val = correct_scores.item()
            print(f"Iter {i+1}: Current prediction = {current_pred}, "
                  f"Target score = {target_score_val:.3f}")
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    ########################################################################
    outputs = model(img)
    target_score = outputs[0, target_y]
   
    if img.grad is not None:
        img.grad.zero_()

    loss = target_score - l2_reg * torch.sum(img * img)
    loss.backward()
