...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i, out_c in enumerate(stem_channels):
            input_c = in_channels if i == 0 else stem_channels[i-1]
            conv = nn.Conv2d(input_c, out_c, kernel_size=3, stride=1, padding=1)
        ...
        # Replace "pass" statement with your code
        self.pred_box = nn.Conv2d(stem_channels[-1], self.num_anchors * 4, 3, 1, 1)
        self.pred_obj = nn.Conv2d(stem_channels[-1], self.num_anchors, 3, 1, 1)
        for layer in [self.pred_box, self.pred_obj]:
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, level_feat in feats_per_fpn_level.items() :
            rpn_feat = self.stem_rpn(level_feat)
            B, _, H, W = level_feat.shape

            object_logits[level_name] = self.pred_obj(rpn_feat).reshape(B, -1) 
            boxreg_deltas[level_name] = self.pred_box(rpn_feat).reshape(B, -1, 4)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            new_height = area / new_width
            # 左下，右上
            x1, x2 = x_c - new_width / 2, x_c + new_width / 2
            y1, y2 = y_c - new_height / 2, y_c + new_height / 2
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    boxes1 = boxes1.unsqueeze(1)  # (M, 1, 4)
    boxes2 = boxes2.unsqueeze(0)  # (1, N, 4)

    b1_x1, b1_y1, b1_x2, b1_y2 = boxes1[..., 0], boxes1[..., 1], boxes1[..., 2], boxes1[..., 3] #(M, 1)
    b2_x1, b2_y1, b2_x2, b2_y2 = boxes2[..., 0], boxes2[..., 1], boxes2[..., 2], boxes2[..., 3]

    inter_x1, inter_y1 = torch.maximum(b1_x1, b2_x1), torch.maximum(b1_y1, b2_y1)
    inter_x2, inter_y2 = torch.minimum(b1_x2, b2_x2), torch.minimum(b1_y2, b2_y2)

    w = torch.clamp(inter_x2 - inter_x1, 0.0)
    h = torch.clamp(inter_y2 - inter_y1, 0.0)
    ...
    ##########################################################################
    return iou
    # return torchvision.ops.box_iou(boxes1, boxes2) #(只需一步)
...
def rcnn_get_deltas_from_anchors(
    ...
    g_x1, g_y1, g_x2, g_y2 = gt_boxes[:, 0], gt_boxes[:, 1], gt_boxes[:, 2], gt_boxes[:, 3]
    g_xc, g_yc, g_w, g_h = (g_x1 + g_x2) / 2 , (g_y1+ g_y2) / 2, (g_x2 - g_x1), (g_y2 - g_y1)
    a_xc, a_yc, a_w, a_h = (a_x1 + a_x2) / 2 , (a_y1+ a_y2) / 2, (a_x2 - a_x1), (a_y2 - a_y1)
    tx = (g_xc - a_xc) / a_w
    ty = (g_yc - a_yc) / a_h
    ...
    # 处理背景和中性anchor：GT boxes中x1坐标为负值表示背景/中性
...
def rcnn_apply_deltas_to_anchors(
    ...
    # Replace "pass" statement with your code
    N = deltas.shape[0]
    output_boxes = torch.zeros(N, 4, device=deltas.device)
    a_x1, a_y1, a_x2, a_y2 = anchors[:, 0], anchors[:, 1], anchors[:, 2], anchors[:, 3]

    xc, yc, a_w, a_h = (a_x1 + a_x2) / 2, (a_y1 + a_y2) / 2 , a_x2 - a_x1, a_y2 - a_y1 
    
    pred_xc = xc + a_w * deltas[:, 0]
    pred_yc = yc + a_h * deltas[:, 1]
    width = a_w * torch.exp(deltas[:, 2])
    height = a_h * torch.exp(deltas[:, 3])

    output_boxes[:, 0], output_boxes[:, 2] = pred_xc - width / 2, pred_xc + width / 2
    output_boxes[:, 1], output_boxes[:, 3] = pred_yc - height / 2, pred_yc + height / 2
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        for level_name in anchors_per_fpn_level:
            anchors_per_fpn_level[level_name] = anchors_per_fpn_level[level_name].to(device)    
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for gt_boxes_per_image in gt_boxes:
            matched_boxes = rcnn_match_anchors_to_gt(anchor_boxes, gt_boxes_per_image, self.anchor_iou_thresholds)
        ...
        if self.training:
            ...
            fg_idx, bg_idx = sample_rpn_training(
                matched_gt_boxes, num_samples=self.batch_size_per_image, fg_fraction=0.5)
            
            # 提取采样样本
            sample_idx = torch.cat([fg_idx, bg_idx])
            sample_gt_boxes = matched_gt_boxes[sample_idx]
            sample_anchors = anchor_boxes[sample_idx]       #RPN对锚框偏移量的预测结果
            sample_pred_logits = pred_obj_logits[sample_idx]
            sample_pred_deltas = pred_boxreg_deltas[sample_idx]

            # 计算锚框到真实框的回归目标（偏移量
            gt_deltas = rcnn_get_deltas_from_anchors(sample_anchors, sample_gt_boxes)
            

            fg_mask = (sample_gt_boxes[:, 4] >= 0)
            loss_obj = F.binary_cross_entropy_with_logits(
                sample_pred_logits, fg_mask.float(),  reduce="none"
            )

            # 仅对前景锚框计算回归损失
            if fg_mask.sum() > 0:
                loss_box = F.l1_loss(
                    sample_pred_deltas[fg_mask], gt_deltas[fg_mask], reduce="none"
                )            
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            level_anchors = anchors_per_fpn_level[level_name] #(H*W*A, 4)
            ...
            # print(f"  锚框形状: {level_anchors.shape} (数量: {level_anchors.shape[0]})")
            # print(f"  偏移量形状: {level_boxreg_deltas.shape} (单批次数量: {level_boxreg_deltas.shape[1]})")
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                output_boxes = rcnn_apply_deltas_to_anchors(
                    batch_boxreg_deltas, level_anchors)
                output_boxes[:, 0] = torch.clamp(output_boxes[:, 0], min=0.0, max=width)
                output_boxes[:, 2] = torch.clamp(output_boxes[:, 2], min=0.0, max=width)
                output_boxes[:, 1] = torch.clamp(output_boxes[:, 1], min=0.0, max=height)
                output_boxes[:, 3] = torch.clamp(output_boxes[:, 3], min=0.0, max=height)
                
                batch_scores = torch.sigmoid(batch_obj_logits)
                k = min(self.pre_nms_topk, batch_scores.numel())
                top_scores, top_indices = torch.topk(batch_scores, k, largest=True)

                top_boxes = output_boxes[top_indices]
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        current_channels = self.backbone.out_channels
        for out_c in stem_channels:
            conv = nn.Conv2d(current_channels, out_c, kernel_size=3, stride=1, padding=1)
            nn.init.normal_(conv.weight, mean=0, std=0.01)
            nn.init.zeros_(conv.bias)
        ...
        linear_layer = (nn.Linear(stem_channels[-1] * roi_size[0] * roi_size[1],
                                   num_classes + 1))
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            # 格式化proposals用于ROI align
            # torchvision.ops.roi_align需要proposals格式为 (batch_idx, x1, y1, x2, y2)
            batch_proposals = []
            for batch_idx, props in  enumerate(level_props):
                # props : (N, 4)
                batch_indices = torch.full((len(props), 1), batch_idx, 
                                           device=props.device) #(N, 1)
                props_with_batch = torch.cat([batch_indices, props], dim=1) #(N, 5)
                batch_proposals.append(props_with_batch)

            if batch_proposals:
                batch_proposals = torch.cat(batch_proposals, dim=0)

                roi_feats = torchvision.ops.roi_align(
                    level_feats, batch_proposals, self.roi_size, 1.0 / level_stride,
                    aligned=True)
                # 特征图是原图经过level_stride倍下采样

            else:
                # (M, C, H, W), M = 0, C：特征图的通道数
        ...
        # Step 1: 采样proposals（25%前景，75%背景）
        fg_idx, bg_idx = sample_rpn_training(
        ...
        sample_idx = torch.cat([fg_idx, bg_idx])
        sample_gt_boxes = matched_gt_boxes[sample_idx]
        sample_pred_logits = pred_cls_logits[sample_idx]

        # 提取类别标签
        gt_class_labels = sample_gt_boxes[:, 4].long() #转换为长整型
        # 标签转换：背景(-1)→0，前景(0-19)→1-20
        gt_class_labels = gt_class_labels + 1

        loss_cls = F.cross_entropy(sample_pred_logits, gt_class_labels)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return {
