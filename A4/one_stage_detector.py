...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        def build_stem(in_channels, stem_channels):
            layers = []
        ...
        stem_cls = build_stem(in_channels, stem_channels)
        stem_box = build_stem(in_channels, stem_channels)
        ...
        # Replace "pass" statement with your code
        pred_in_channel = stem_channels[-1]
        self.pred_cls = nn.Conv2d(pred_in_channel, num_classes, kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(pred_in_channel, 4, 3, 1, 1)
        self.pred_ctr = nn.Conv2d(pred_in_channel, 1, 3, 1, 1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for p, p_feat in feats_per_fpn_level.items():
            cls_feat = self.stem_cls(p_feat)
            box_feat = self.stem_box(p_feat)
            ...
            class_logits[p] = cls_logit.permute(0, 2, 3, 1).reshape(B, H*W, -1)
            boxreg_deltas[p] = box_deltas.permute(0, 2, 3, 1).reshape(B, H*W, -1)
            centerness_logits[p] = ctr_logit.permute(0, 2, 3, 1).reshape(B, H*W, -1)
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.unsqueeze(dim=2).unbind(dim=1)   #(N, 1)
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2)
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2)    #(N, M, 4)
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    N = locations.shape[0]
    deltas = torch.zeros(N, 4, device=locations.device, dtype=locations.dtype)
    gt_x1, gt_y1, gt_x2, gt_y2 = gt_boxes[:, 0], gt_boxes[:, 1], gt_boxes[:, 2], gt_boxes[:, 3]
    x_c, y_c = locations[:, 0], locations[:, 1]
    deltas = torch.stack([x_c-gt_x1, y_c-gt_y1, gt_x2-x_c, gt_y2-y_c], dim=1) / stride
    
    is_background = (gt_boxes[:, :4] < 0).any(dim=1)    #(N, )

    deltas[is_background] = -1.0
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    # Replace "pass" statement with your code
    N = deltas.shape[0]
    output_boxes = torch.zeros(N, 4, device=deltas.device, dtype=deltas.dtype)
    x_c, y_c = locations[:, 0], locations[:, 1]
    deltas_unnorm = torch.where(deltas >=0, deltas, 0) * stride
    
    x_1, x_2 = x_c - deltas_unnorm[:, 0], x_c + deltas_unnorm[:, 2]
    y_1, y_2 = y_c - deltas_unnorm[:, 1], y_c + deltas_unnorm[:, 3]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    left, top, right, bottm = deltas[:, 0], deltas[:, 1], deltas[:, 2], deltas[:, 3]

    min_lr, max_lr = torch.min(left, right), torch.max(left, right)
    min_tb, max_tb = torch.min(top,bottm), torch.max(top,bottm)
    centerness = torch.sqrt(min_lr * min_tb / (max_lr * max_tb)) 
    
    is_background = (deltas < 0).any(dim=1)
    centerness[is_background] = -1.0
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for gt_boxes_per_image in gt_boxes:
            # 去除填充的GT框（坐标为-1的框）
            valid_gt = gt_boxes_per_image[ gt_boxes_per_image[:, 4] >= 0 ]

            # 如果没有有效的GT框，创建全背景的匹配结果
            if len(valid_gt) == 0:
                matched_boxes_per_image = {}
                for level_name, locations in locations_per_fpn_level.items():
                    num_locations = locations.shape[0]
                    # 创建全背景框：(-1, -1, -1, -1, -1)
                    background_boxes = torch.full(
                        (num_locations, 5), -1.0,
                        device=gt_boxes_per_image.device,
                        dtype=gt_boxes_per_image.dtype,
                    )
                    matched_boxes_per_image[level_name] = background_boxes
            else:
                # 有有效GT框，正常进行匹配
                matched_boxes_per_image = fcos_match_locations_to_gt(locations_per_fpn_level,
                                                        self.backbone.fpn_strides, valid_gt) #Dictionary with same keys as shape_per_fpn_level
        ...
        matched_gt_deltas = []
        # Replace "pass" statement with your code
        for matched_box in matched_gt_boxes:
            deltas_per_image = {}
            for level_name in matched_box.keys():
                level_locations = locations_per_fpn_level[level_name]
                level_stride = self.backbone.fpn_strides[level_name]
                level_matched_boxes = matched_box[level_name]
                deltas = fcos_get_deltas_from_locations(level_locations, level_matched_boxes, level_stride)
        ...
        # Replace "pass" statement with your code
        B, N = matched_gt_boxes.shape[:2]       # B=batch_size, N=total_locations
        gt_classes = matched_gt_boxes[:, :, 4].long()  # 提取类别标签  shape: (B, N) 

        is_foreground = (gt_classes >= 0 )   # (B, N) 前景掩码
        cls_targets = torch.zeros(B, N, self.num_classes, device=images.device)
        # cls_targets[is_foreground, gt_classes[is_foreground].long()] = 1.0

        foreground_classes = gt_classes[is_foreground].long() 
        # 生成独热编码并赋值到前景位置
        cls_targets[is_foreground] = torch.nn.functional.one_hot(
            foreground_classes, 
            num_classes=self.num_classes
        ).float()

        loss_cls = sigmoid_focal_loss(pred_cls_logits, cls_targets, reduction="none")
        ...
        ctr_targets = []
        for i in range(B):
            batch_ctr = fcos_make_centerness_targets(matched_gt_deltas[i])
            ctr_targets.append(batch_ctr)
        
        ctr_targets = torch.stack(ctr_targets)  #(B, N)
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.squeeze(-1),
                                                       ctr_targets, reduction="none")
        
        loss_ctr[ctr_targets < 0 ] *= 0.0
